...
# Local import
from .utils import orcl, DecimalEncoder, get_soup, normalize
from .getpath import app_root
...
BASE_DIR = app_root() # os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
...
def save(rates, year, month):
    ...
    for date in rates.keys():
        ...
        try:
            ...
            raise ValueError(f"No se puede crear el directorio. -.ERR: {err_app}")
...
def savedb(rates, year, month):
    ...
    if latest:
            ...
            db = orcl()
            db.conectar()
            j_body = json.dumps(latest, cls=DecimalEncoder)
